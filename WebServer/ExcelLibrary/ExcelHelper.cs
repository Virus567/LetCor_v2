    ...
    public class ExcelHelper : IDisposable
    {
        ...
        public void SetParticipant(List<Participant> participants, int childrenAmount, TouristGroup group)
        {
            try
            {
                ...
                _worksheet1.Cells[4, 6] = "Количество детей до 5 лет:";
                _worksheet1.Cells[5, 6] = "Количество детей до 10 лет:";
                _worksheet1.Cells[6, 6] = "Количество детей до 14 лет:";
                _worksheet1.Cells[2, 7] = participants.Count;
                _worksheet1.Cells[3, 7] = childrenAmount;
                _worksheet1.Cells[4, 7] = group.GetChildren5();
                _worksheet1.Cells[5, 7] = group.GetChildren10(); ;
                _worksheet1.Cells[6, 7] = group.GetChildren14(); ;
        ...
        public void SetEvents(List<Event.EventView> evnts)
        {
            try
            {
                var evnets = evnts.Where(e=>e.TypeEvent!="Фрахтование").OrderBy(e => DateTime.Parse(e.Date)).ToList();
                _worksheet1 = (Excel.Worksheet)_workbook.Worksheets.get_Item(1);
                _worksheet1.Name = "Отчет по мероприятиям";
                _worksheet1.PageSetup.Orientation = Excel.XlPageOrientation.xlLandscape;
                int peopleAmount = 0;
                double totalPrice = 0;

                object[,] eventsExport = new object[evnets.Count, 8];

                for (int i = 0; i < evnets.Count; i++)
                {
                    eventsExport[i, 1] = evnets[i].Date;
                    eventsExport[i, 2] = evnets[i].Time;
                    eventsExport[i, 3] = evnets[i].Status;
                    eventsExport[i, 4] = evnets[i].PeopleAmount;
                    eventsExport[i, 5] = evnets[i].School;
                    eventsExport[i, 6] = evnets[i].Class;
                    eventsExport[i, 7] = evnets[i].TotalPrice;
                    peopleAmount += evnets[i].PeopleAmount;
                    totalPrice += evnets[i].TotalPrice;
                }
                for (int i = 0; i < evnets.Count; i++)
                {
                    eventsExport[i, 0] = i+1;
                }

                if (evnets.Count != 0)
                {
                    _excelRange = _worksheet1.get_Range("A2", Missing.Value);
                    _excelRange = _excelRange.get_Resize(evnets.Count, 8);
                    _excelRange.set_Value(Missing.Value, eventsExport);
                    _excelRange.Columns.AutoFit();
                    _worksheet1.Cells[1, 1] = "№";
                    _worksheet1.Cells[1, 2] = "Дата";
                    _worksheet1.Cells[1, 3] = "Время";
                    _worksheet1.Cells[1, 4] = "Статус";
                    _worksheet1.Cells[1, 5] = "Количество человек";
                    _worksheet1.Cells[1, 6] = "Школа";
                    _worksheet1.Cells[1, 7] = "Класс";
                    _worksheet1.Cells[1, 8] = "Общая цена";
                    _worksheet1.Cells[2, 9] = "Всего мероприятий:";
                    _worksheet1.Cells[3, 9] = "Общее количество человек:";
                    _worksheet1.Cells[4, 9] = "Общая выручка:";
                    _worksheet1.Cells[2, 10] = evnets.Count;
                    _worksheet1.Cells[3, 10] = peopleAmount;
                    _worksheet1.Cells[4, 10] = totalPrice;
                    _worksheet1.Columns.AutoFit();
                    _excel.Visible = true;
                }
                
                
                var charters = evnts.Where(e=>e.TypeEvent =="Фрахтование").OrderBy(e => DateTime.Parse(e.Date)).ToList();
                _worksheet2 = (Excel.Worksheet)_workbook.Worksheets.get_Item(2);
                _worksheet2.Name = "Отчет по фрахтованиям";
                _worksheet2.PageSetup.Orientation = Excel.XlPageOrientation.xlLandscape;
                int peopleAmount1 = 0;
                double totalPrice1 = 0;

                object[,] eventsExport1 = new object[charters.Count, 7];

                for (int i = 0; i < charters.Count; i++)
                {
                    eventsExport1[i, 1] = charters[i].Date;
                    eventsExport1[i, 2] = charters[i].Time;
                    eventsExport1[i, 3] = charters[i].Status;
                    eventsExport1[i, 4] = charters[i].PeopleAmount;
                    eventsExport1[i, 5] = charters[i].School;
                    eventsExport1[i, 6] = charters[i].Price;
                    peopleAmount1 += charters[i].PeopleAmount;
                    totalPrice1 += charters[i].Price;
                }
                for (int i = 0; i < charters.Count; i++)
                {
                    eventsExport1[i, 0] = i+1;
                }

                if (charters.Count != 0)
                {
                    _excelRange = _worksheet2.get_Range("A2", Missing.Value);
                    _excelRange = _excelRange.get_Resize(charters.Count, 7);
                    _excelRange.set_Value(Missing.Value, eventsExport1);
                    _excelRange.Columns.AutoFit();
                    _worksheet2.Cells[1, 1] = "№";
                    _worksheet2.Cells[1, 2] = "Дата";
                    _worksheet2.Cells[1, 3] = "Время";
                    _worksheet2.Cells[1, 4] = "Статус";
                    _worksheet2.Cells[1, 5] = "Количество человек";
                    _worksheet2.Cells[1, 6] = "Школа";
                    _worksheet2.Cells[1, 7] = "Класс";
                    _worksheet2.Cells[1, 8] = "Общая цена";
                    _worksheet2.Cells[2, 9] = "Всего мероприятий:";
                    _worksheet2.Cells[3, 9] = "Общее количество человек:";
                    _worksheet2.Cells[4, 9] = "Общая выручка:";
                    _worksheet2.Cells[2, 10] = charters.Count;
                    _worksheet2.Cells[3, 10] = peopleAmount1;
                    _worksheet2.Cells[4, 10] = totalPrice1;
                    _worksheet2.Columns.AutoFit();
                    _excel.Visible = true;
                }
            }
